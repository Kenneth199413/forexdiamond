...
$.createOOPlugin = function(plgName, extend, proto) {
  var Id = $.createOOPlugin.Id = $.createOOPlugin.Id || 0,
      Class = $.createOOPlugin.Class = $.createOOPlugin.Class || {};
  // create class
  if ($.fn[plgName]) return console.error(plgName+" plugin already exists!");
  if (Class[extend]) {
    extend = {'super': Class[extend].prototype};
    for (var prop in extend['super'])
      if (typeof extend['super'][prop] === "function") extend[prop] =
        new Function("this['super']."+prop+".apply(this, arguments)");
      else if (typeof extend['super'][prop] === "object")
        $.extend(true, extend[prop], extend['super'][prop]);
      else extend[prop] = extend['super'][prop];
  }
...
$.createOOPlugin("ialTextfield", "ialElem", {
  ...
  onBlur: function() {
    ...
    if (result && this.$load) {
      ...
      $.ajax({
        type: "POST",
        dataType: "json",
        url: ologin.base + "?ialCheck=" + this.$input.attr("name"),
        data: "value=" + value,
...
$.createOOPlugin("ialPassword1", "ialElem", {
  validClass: "ial-correct",
  min: 4,
  tmpl:
    '<label data-attr="label required" class="smallTxt" />'+
...
$.createOOPlugin("ialSubmitLogin", "ialSubmit", {
  onSubmitSuccess: function(resp) {
    this['super'].onSubmitSuccess.call(this, resp);
    if (!resp.error) {
...
$.createOOPlugin("ialWindow", {
  ...
  constructor: function(params) {
    ...
    $(window).on("ready resize", $.proxy(this, "initPosition"));
    // init node
    this.$node
      .appendTo(document.body)
  ...
  initPosition: function() {
    ...
    if (this.popupCenter) {
      // popup center
      pos.top = ($(window).height() - wndH) / 2;
      pos.left = ($(window).width() - wndW) / 2;
      pos.marginTop = 0;
      this.$arrow.css("display", "none");
      ...
      if (btnP.left + btnW/2 < $(window).width()/2) {
        // float left
...
$.createOOPlugin("ialOAuth", {
  ...
  open: function() {
    var sw = window.open("", "Login", "width=450,height=500,"+
  ...
  initLoading: function(sw) {
    ...
    ||  navigator.userAgent.indexOf('iPhone') >= 0) {
      sw.document.write('<br />'+
        '<h2 style="margin-left:160px; font-family:Verdana; color:#444;'+
        ' text-shadow:1px 1px 2px #aaa">Loading..</h2>');
      sw.load = sw.document.body.children[1];
      sw.setInterval("load.innerHTML+='.'", this.delay);
    } else {
      var ss, s, j, i;
      sw.document.write(
